...
function Register-CollectionOnlyTask {
    ...
    $child = @(('"{0}"' -f $Python), '-X', 'utf8', ('"{0}"' -f $ChildScript)) + $ChildArgs
    $runnerArgs = @(
    ...
    if ($ConsumerStatusFile) { $runnerArgs = $runnerArgs[0..($runnerArgs.Count - $child.Count - 1)] + @('--consumer-status-json', ('"{0}"' -f $ConsumerStatusFile), '--') + $child }
    else { $runnerArgs = $runnerArgs[0..($runnerArgs.Count - $child.Count - 1)] + @('--') + $child }
...
Register-CollectionOnlyTask -Name 'cs-archival-collection' -Project 'cs-predictor' -Python (Join-Path $csRoot '.venv\Scripts\python.exe') -ChildScript (Join-Path $csRoot 'scripts\run_archival_collection.py') -ChildArgs @('--input',('"{0}"' -f (Join-Path $csRoot 'data\collection_only\upstream_events.json')),'--status-output',('"{0}"' -f (Join-Path $runtimeRoot 'cs-predictor\cs-archival-collection\cs-archival-collection.consumer-status.json'))) -ConsumerStatusFile (Join-Path $runtimeRoot 'cs-predictor\cs-archival-collection\cs-archival-collection.consumer-status.json') -Schedule 'hourly source-file check' -TimeoutSeconds 300 -Trigger (New-ScheduledTaskTrigger -Once -At (Get-Date).AddMinutes(5) -RepetitionInterval (New-TimeSpan -Hours 1))
